...
export class AvalancheProvider implements BlockchainProvider {
  private config: any;
  ...
  async connect(): Promise<void> {
    try {
      ...
      throw new NetworkError('Failed to connect to Avalanche', error);
    }
  }
  ...
  async getBalance(address: string): Promise<string> {
    ...
    try {
      return '1000000000000000000'; // 1 AVAX
    } catch (error) {
